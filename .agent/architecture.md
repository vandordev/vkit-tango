...
- `apps/api` owns the Elysia app and routes under `/api`; `apps/web/src/server.ts` embeds that app through the TanStack Start server entry for the semi-monolith deployment.
...
- `packages/application` owns mutation business rules and transactions. It must not import Elysia or TanStack Start.
