...
const requiredByFile: Record<string, string[]> = {
  ...
  ".agent/worker.md": ["Fx", "internal/contract.Command", "apps/scheduler", "idempotent", "versioned", "retry"],
  ".agent/config.md": ["Fx", "scheduler", "least data"],
  ".agent/web.md": ["TanStack Start", "Hey API", "/api/v1", "unknown", "any"],
  ".agent/repository-map.md": ["internal/generated/fx", "internal/lib", "samber/lo"],
  ".agent/workflows.md": ["OperationID", "one command", "idempotent"],
  ".agent/verification.md": ["task sync", "task quality", "task build"],
};
